describe("%||% operator", {
  ...
  it("handles numeric NA and non-finite values", {
  ...
  it("handles character NA and other types", {
  ...
  it("handles vector inputs correctly", {
    ...
    # Vector with some NA values should return vector (not fallback)
    expect_equal(c(1, NA, 3) %||% 99, c(1, NA, 3))
    expect_equal(c("a", NA, "c") %||% "default", c("a", NA, "c"))
  })

  it("handles edge cases that caused original bug", {
...
describe("calculate_perceptual_distances()", {
  ...
  it("handles single color with NA", {
  ...
  it("handles invalid input (duplicate test)", {
    # Not a matrix
    result_vector <- calculate_perceptual_distances(c(1, 2, 3))
    expect_true(all(is.na(result_vector)))
...
describe("ERROR CONDITION TESTS - Testing validation function error paths", {
  it("generate_palette validation catches invalid aesthetic parameters", {
...
describe(".merge_aesthetic_config()", {
...
describe("validate_inputs()", {
  ...
  it("validates include_colors parameter", {
    valid_params <- list(
      n = 3,
      include_colors = NULL,
      init_lightness_bounds = c(0.2, 0.8),
...
describe(".handle_no_free_colors()", {
  it("works with fixed colors only", {
