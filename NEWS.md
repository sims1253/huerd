...
- Removed more dead internal code and doc drift: the unused `.calculate_luminance()` helper and the empty `R/data.R`; `.print_color_with_swatch()` / `print_color_vector()` roxygen no longer claims swatches (text labels only); `.handle_no_free_colors()` roxygen documents `generation_metadata` and drops the nonexistent `optimize_for`
