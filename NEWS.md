...
- Removed more dead internal code and doc drift: the unused `.calculate_luminance()` helper and the empty `R/data.R`; the text-only color print helper is renamed `.print_color_with_swatch()` -> `.print_color_label()` and its roxygen (plus `print_color_vector()`'s) no longer claims swatches; `.handle_no_free_colors()` roxygen documents `generation_metadata` and drops the nonexistent `optimize_for`
