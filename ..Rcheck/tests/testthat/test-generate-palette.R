...
describe("generate_palette()", {
  describe("basic functionality", {
    it("creates palette with specified number of colors", {
  ...
  describe("edge cases", {
    it("handles single color and zero colors", {
    ...
    it("returns colors sorted by brightness (lightness)", {
  ...
  describe("fixed colors", {
    it("includes fixed colors unchanged", {
    ...
    it("works with all fixed colors", {
  ...
  describe("initialization methods", {
    it("accepts different initialization methods", {
  ...
  describe("attributes and metrics", {
    it("respects return_metrics parameter", {
      # With metrics
  ...
  describe("weights parameter", {
    ...
    it("validates weights parameter", {
    ...
    it("backward compatibility with weights", {
  ...
  describe("nloptr_cobyla optimizer", {
    it("uses pure minimax optimization", {
    ...
    it("accepts optimizer parameter", {
    ...
    it("validates optimizer parameter", {
    ...
    it("optimizer parameter is backward compatible", {
  ...
  describe("sann optimizer", {
    it("accepts sann optimizer", {
      # Test with sann optimizer
      palette_sann <- generate_palette(
        n = 3,
        optimizer = "sann",
        progress = FALSE
      )
  ...
  describe("nlopt_direct optimizer", {
    ...
    it("provides scientific reproducibility", {
      ...
      expect_equal(
        details1$final_objective_value,
        details2$final_objective_value
      )
      expect_equal(
        details2$final_objective_value,
        details3$final_objective_value
      )
    })
  })

  describe("nlopt_neldermead optimizer", {
    it("accepts nlopt_neldermead optimizer", {
      ...
      expect_true(
        "optimization_details" %in% names(attributes(palette_neldermead))
      )
    ...
    it("produces reasonable consistency", {
      ...
      # Local optimizer may get stuck in suboptimal solutions
      # Just verify results are valid (not NA) and exist
      expect_true(!is.na(metrics1$distances$performance_ratio))
      expect_true(!is.na(metrics2$distances$performance_ratio))
      expect_true(is.numeric(metrics1$distances$performance_ratio))
      expect_true(is.numeric(metrics2$distances$performance_ratio))
    })

    it("validates nlopt_neldermead optimizer", {
    ...
    it("provides robust local optimization", {
  ...
  describe("smooth optimization (v0.5.0)", {
    it("works with smooth_repulsion weights", {
      palette <- generate_palette(
        n = 4,
      ...
      # Check metadata
      metadata <- attr(palette, "generation_metadata")
      expect_equal(metadata$weights, c(smooth_repulsion = 1))
      expect_equal(metadata$optimizer, "nlopt_lbfgs")
    })

    it("works with smooth_logsumexp weights", {
      palette <- generate_palette(
        n = 4,
      ...
      # Check metadata
      metadata <- attr(palette, "generation_metadata")
      expect_equal(metadata$weights, c(smooth_logsumexp = 1))
      expect_equal(metadata$optimizer, "nlopt_lbfgs")
    })

    it("smooth optimization produces different results for different objectives", {
      # This is the critical integration test for the bug we fixed
      fixed_colors <- c("#FF0000", "#0000FF")

      palette_repulsion <- generate_palette(
        n = 5,
        include_colors = fixed_colors,
        weights = c(smooth_repulsion = 1),
        optimizer = "nlopt_lbfgs",
      ...
      palette_logsumexp <- generate_palette(
        n = 5,
        include_colors = fixed_colors,
        weights = c(smooth_logsumexp = 1),
        optimizer = "nlopt_lbfgs",
      ...
      # Both should include fixed colors
      expect_true(all(fixed_colors %in% palette_repulsion))
      expect_true(all(fixed_colors %in% palette_logsumexp))

      # Critical test: Results should be different (this was the bug)
      expect_false(identical(palette_repulsion, palette_logsumexp))
    })

    it("smooth optimization metadata is preserved in reproduction", {
      # Test reproducibility integration with smooth optimization
      ...
      reproduced_palette <- reproduce_palette(
        original_palette,
        progress = FALSE
      )

      expect_true(inherits(reproduced_palette, "huerd_palette"))
      expect_length(reproduced_palette, 4)

      # Metadata should match
      original_metadata <- attr(original_palette, "generation_metadata")
      reproduced_metadata <- attr(reproduced_palette, "generation_metadata")

      expect_equal(original_metadata$weights, reproduced_metadata$weights)
      expect_equal(original_metadata$optimizer, reproduced_metadata$optimizer)
      expect_equal(original_metadata$n_colors, reproduced_metadata$n_colors)
    })

    it("L-BFGS optimizer requires smooth weights", {
      # Test that L-BFGS works with smooth objectives
      expect_no_error({
        palette <- generate_palette(
          n = 3,
          weights = c(smooth_repulsion = 1),
    ...
    it("smooth optimization handles edge cases gracefully", {
